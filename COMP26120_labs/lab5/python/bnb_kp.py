...
class bnb(knapsack):
    ...
    def branch_and_bound(self, final_sol):
        ...
        self.frac_bound(emptysol, emptysol.fixed)
        upperbound = emptysol.bound
         # put current_best = to its value
        currentbest = emptysol.val
        # store it in the priority queue
        self.insert(emptysol)
        ...
        #       if value > current_best, set current_best to it and set bound, and copy child to final_sol
        #       add child to the queue
        # RETURN
  
        # YOUR CODE GOES HERE

        while self.QueueSize > 0 and currentbest < upperbound:
            ...
            child2 = parent.copy()
            child2.fixed += 1
            child2.solution_vec[child2.fixed]=False
            for each in [child1, child2] :
                self.frac_bound(each, each.fixed)
                if(each.val >= 0):
                    if (each.val >= currentbest):
                        currentbest = each.val
                        print(currentbest)
                        upperbound = each.bound
                        self.copy_array(each.solution_vec, final_sol)
                    self.insert(each)
        return final_sol
